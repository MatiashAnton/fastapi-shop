...
from backend.app.schemas.cart import CartItemCreate, CartResponse, CartItem, CartItemUpdate
...
class CartService:
	...
	def add_item_to_cart(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
	...
	def update_item_in_cart(self, cart_data: Dict[int, int], item: CartItemUpdate) -> Dict[int, int]:
	...
	def remove_item_from_cart(self, cart_data: Dict[int, int], product_id: int) -> Dict[int, int]:
	...
	def get_cart_details(self, cart_data: Dict[int, int]) -> CartResponse:
		...
		return CartResponse(items=cart_items, total_quantity=total_quantity, total_price=round(total_price, 2))
